...
data = pd.read_csv('D:/Lunch/MLProject/oasis_longitudinal.csv')
...
my_pipeline = make_pipeline(my_imputer,RandomForestRegressor(random_state=99))
scores = cross_val_score(my_pipeline,X,y,scoring='neg_mean_absolute_error')
print('Random Forest with Cross-Validation, MAE: %2f' %(-1 * scores.mean()))



my_pipeline = make_pipeline(my_imputer,XGBRegressor())
